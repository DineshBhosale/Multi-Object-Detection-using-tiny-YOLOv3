...
def create_yolov3_model(
  ...
  #   Layer  9 => 11
    ...
    x = MaxPooling2D(pool_size=(2, 2), strides=2, padding='same', data_format=None)(x)
     #Layer 12 => 15
    x = _conv_block(x, [{'filter': 128, 'kernel': 3, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 6}], do_skip=False)
    x = MaxPooling2D(pool_size=(2, 2), strides=2, padding='same', data_format=None)(x)
    
    x = _conv_block(x, [{'filter': 256, 'kernel': 3, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 8}], do_skip=False)
    skip_8=x
    x = MaxPooling2D(pool_size=(2, 2), strides=2, padding='same', data_format=None)(x)
    
    x = _conv_block(x, [{'filter': 512, 'kernel': 3, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 10}], do_skip=False)
    x = MaxPooling2D(pool_size=(2, 2), strides=1, padding='same', data_format=None)(x)
    
    x = _conv_block(x, [{'filter': 1024, 'kernel': 3, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 12}], do_skip=False)
    
    
    x = _conv_block(x, [{'filter': 256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 13}], do_skip=False)
    ...
    # Layer 83 => 86
    x = _conv_block(x, [{'filter': 128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 18}], do_skip=False)
    x = UpSampling2D(2)(x)
